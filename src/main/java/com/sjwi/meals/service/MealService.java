...
public class MealService {
  ...
  public Set<Integer> getItemIngredientsIdsToAdd(Set<String> rawIngredients) {
    ...
    rawIngredients.forEach(
        i -> {
          if (!NumberUtils.isCreatable(i)) ingredientsToCreate.add(i.toLowerCase());
          else existingIngredients.add(Integer.parseInt(i));
        });
  ...
  public Set<Integer> getMealTagIdsToAdd(Set<String> rawTags) {
    ...
    rawTags.forEach(
        t -> {
          if (!NumberUtils.isCreatable(t)) tagsToCreate.add(t.toLowerCase());
          else existingTags.add(Integer.parseInt(t));
        });
